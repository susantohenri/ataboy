...
function formInit(scope) {
    ...
    scope.find('select').not('.select2-hidden-accessible').each(function () {
        if ($(this).is('[name^="BarangKeluar_satuan"]')) {
            ...
            $(this).select2({
                ajax: {
                    ...
                    data: function (params) {
                        var query = {
                            term: params.term,
                            barang: $(this).parent().parent().find('[name^="BarangKeluar_barang["]').val()
                        }
                        return query;
                    },
        ...
        } else if ($(this).is('[name="pengajuan"]')) {
            var model = $(this).attr('data-model')
            var field = $(this).attr('data-field')
            $(this).select2({
                ajax: {
            ...
            $(this).change(function () {
                ...
                $.get(`${site_url}PengajuanBarang/getUuidByPengajuan/${pengajuan}`, function (result) {
                    $('.btn-delete').click()
...
//                        console.log(site_url + 'BarangKeluar/subformcreate/' + uuids[index])
                        $.get(site_url + 'BarangKeluar/subformcreate/' + uuids[index], function (form) {
                            ...
                            index++
                            if (uuids[index])
                                generateSubForm(index)
                        })
                    }
                })
            })
        } else if ($(this).is('[name^="BarangKeluar_barang["]')) {
            var model = $(this).attr('data-model')
            var field = $(this).attr('data-field')
            $(this).select2({
                ajax: {
            ...
            $(this).change(function () {
                let jumlah = $(this).parent().parent().find('[name^="BarangKeluar_jumlah"]')
                let satuan = $(this).parent().parent().find('[name^="BarangKeluar_satuan"]')
                jumlah.val('')
                satuan.val('')
