...
function formInit(scope) {
  ...
  scope.find('select').not('.select2-hidden-accessible').each(function () {
    if ($(this).is('[name^="BarangMasuk_satuan"]')) {
      ...
      $(this).select2({
        ajax: {
          ...
          data: function (params) {
            var query = {
              search: params.term,
              barang: $(this).parent().parent().find('[name^="BarangMasuk_barang["]').val()
            }
            return query;
          },
      ...
      $(this).change(function () {
        let jumlah = $(this).parent().parent().find('[name^="BarangMasuk_jumlah"]')
        let satuan = $(this).parent().parent().find('[name^="BarangMasuk_satuan"]')
        jumlah.val('')
        satuan.val('')
